...
export function balance(account) {
  return databaseController.checkAvailableBalance(account).then(value => {
    return { value };
  });
}
...
export function deposit(data) {
  if (checkData(data)) {
    return checkAllowedBanknotes(data.banknotes).then(allowed => {
      //console.log(allowed);
      if (allowed) {
  ...
  function checkData(data) {
    return (
      ...
      data.targetAccount &&
      Number(data.value) ===
        data.banknotes.reduce((sum, note) => {
...
export function withdraw(data) {
  ...
  if (checkData(data)) {
    ...
    return databaseController
      .checkAvailableBalance(data.account, data.value)
      .then(goodToGo => {
        if (goodToGo) {
          // generates statement
          return databaseController.generateStatement(data).then(statement => {
  ...
  function checkData(data) {
    return !Number.isNaN(Number(data.value)) && data.account;
  }
}
  ...
  return databaseController
    ...
    .then(minimum => {
      ...
      if (minimum) {
        ...
        return databaseController.bulkOp("Banknote", bulkOp).then(() => {
          return minimum;
        });
      } else {
        throw new Error(
          "Insufficient banknotes or Invalid banknotes calculation"
        );
      }
    });
}
...
function getMinimumBanknotes(notes, count, value) {
  ...
  if (finalResult) {
    for (var i = 0; i < notesSize; i++) {
      if (count[i] < finalResult[notes[i]]) return false;
    }
    return finalResult;
  } else {
    return false;
